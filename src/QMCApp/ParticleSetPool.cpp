...
ParticleSet* ParticleSetPool::createInterfaceParticleSet(xmlNodePtr cur, ParticleSet* qp)
{
  ...
  if(ions==0)
  {
    ...
    myComm->barrier();
    cout<<"ION interface->bcastParticleSet() myComm rank()="<<myComm->rank()<<" size()="<<myComm->size()<<endl;
    interface->bcastParticleSet(*ions);
    myComm->barrier();
    app_log()<<"ION interface---done with bcastParticleSet()\n";
  //    hid_t h5=-1;
//    if(myComm->rank()==0)
//      h5 = H5Fopen(inname.c_str(),H5F_ACC_RDONLY,H5P_DEFAULT);
   // ESHDFIonsParser ap(*ions,h5,myComm);
  //  ap.getParticleSet(*ions);
 //   ap.bcastParticleSet(*ions);
    //ap.put(cur);
    expandSuperCell(*ions,tilematrix);
    myComm->barrier();
    app_log()<<"ION expanded supercell rank="<<myComm->rank()<<endl;
    ions->resetGroups();
    myComm->barrier();
    app_log()<<"ION resetGroups rank="<<myComm->rank()<<endl;
    ions->createSK();
    myComm->barrier();
    app_log()<<"ION created SK rank="<<myComm->rank()<<endl;
...
//    ap.expand(eshdf_tilematrix);
  ...
  if(qp==0)
  {
    ...
    cout<<"interface->ELECTRONbcastParticleSet() myComm rank()="<<myComm->rank()<<" size()="<<myComm->size()<<endl; 
    myComm->barrier();

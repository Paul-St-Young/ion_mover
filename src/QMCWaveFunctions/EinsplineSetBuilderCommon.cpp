...
EinsplineSetBuilder::CheckLattice()
{
  ...
  app_log()<<"CheckLattice()_____\n";
  update_token(__FILE__,__LINE__,"CheckLattice");
  double diff=0.0;
  cout<<"CheckLattice 103.  Rank = "<< OHMMS::Controller->rank()<<endl;
  OHMMS::Controller->barrier();
  for (int i=0; i<OHMMS_DIM; i++)
    for (int j=0; j<OHMMS_DIM; j++)
      diff=std::max(diff,std::abs(SuperLattice(i,j) - TargetPtcl.Lattice.R(i,j)));

  cout<<"CheckLattice 109.  Rank = "<< OHMMS::Controller->rank()<<endl;
  OHMMS::Controller->barrier();
  if(diff>MatchingTol)
  {
    ostringstream o;
  ...
  cout<<"CheckLattice 128.  Rank = "<< OHMMS::Controller->rank()<<endl;
  OHMMS::Controller->barrier();
  update_token(__FILE__,__LINE__,"CheckLattice");
  return true;
}

...
//  app_log()<<"Building interface in WaveFunctionFactory\n";
//  interface=new ESPWSCFInterface(c);
}
...
{
  app_log()<<"WaveFunctionFactory::setInterface: intfc="<<intfc<<endl;
  interface=intfc;
  app_log()<<"WaveFunctionFactory::setInterface: interface="<<interface<<endl;
}
...
//  interface->initialize(); 
  ...
  if(targetPsi==0) //allocate targetPsi and set the name
  {
    ...
    app_log()<<"Lets see if stuff goes down here...\n";
    targetPsi->setMassTerm(*targetPtcl);
    app_log()<<"WaveFunctionFactory::build made it through the mass assignment\n";
  }
  cur = cur->children;
  bool success=true;
  while(cur != NULL)
  {
    ...
    if(cname =="sposet_builder")
    {
      ...
      basisFactory.setInterface(interface);
...
//#endif /* QMC_BUILD_LEVEL>1 */
  ...
  detbuilder->setInterface(interface);
  detbuilder->put(cur);

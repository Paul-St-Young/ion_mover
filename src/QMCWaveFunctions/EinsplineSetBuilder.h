...
struct H5OrbSet
{
  ...
  int NumOrbs;
  ///Way to sort the orbitals.  Energy?  Band?  etc.
  int SortBands;
  ...
  H5OrbSet (string name, int spinSet, int numOrbs) :
    FileName(name), SpinSet(spinSet), NumOrbs(numOrbs), SortBands(1)
  { }
  H5OrbSet (string name, int spinSet, int numOrbs, int sortBands) :
    FileName(name), SpinSet(spinSet), NumOrbs(numOrbs), SortBands(sortBands)
  { }
  H5OrbSet()
  { }
};
...
public:
  void setInterface(InterfaceBase* ifc);
  ...
  typedef std::map<H5OrbSet,SPOSetBase*,H5OrbSet>::iterator SPOSetMap_iter;
  ///constructor
...
//  inline void setInterface(ESInterfaceBase* ibase){interface=ibase;}; //set the interface.  

  bool ReadOrbitalInfo_Interface();
  bool ReadGvectors_Interface();
  SPOSetBase* createSPOSetFromInterface(xmlNodePtr cur);
  void OccupyBands_Interface(int spin, int sortBands, int numOrbs);

  void updateIons(SPOSetBase* spo, ParticleSet& P);
  void updateIons(SPOSetBase* spo, ParticleSet& P, int iat);
//#endif

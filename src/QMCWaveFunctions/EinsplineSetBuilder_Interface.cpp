...
void EinsplineSetBuilder::setInterface(InterfaceBase* ifc)
{
  esinterface=dynamic_cast<ESInterfaceBase*>(ifc);
}

void EinsplineSetBuilder::updateIons(SPOSetBase* spo, ParticleSet& ions)
{
 // app_log()<<"Doop!  In EinsplineSetBuilder\n";
//  return;
  esinterface->setIonPositions(ions.R);
  esinterface->update();

  //now we iterate over and update all the spin SPOSets we have build
  for(SPOSetMap_iter it=SPOSetMap.begin(); it!=SPOSetMap.end(); it++)
  {
    H5OrbSet aset = it->first;
    SPOSetBase* spo = it->second;
    //Update the orbital occupations.  
    OccupyBands_Interface(aset.SpinSet, aset.NumOrbs, aset.SortBands);
    MixedSplineReader->read_spline_set(spo,aset.SpinSet);

  }

//  mixedsplinereader->updateIons(spo,ions);  
//  OccupyBands_Interface(esinterface, spin, sortBands, numOrbs);
  

  
}

void EinsplineSetBuilder::updateIons(SPOSetBase* spo, ParticleSet& ions, int iat)
{
  updateIons(spo, ions, iat);
}

bool EinsplineSetBuilder::ReadOrbitalInfo_Interface()
{
  update_token(__FILE__,__LINE__,"ReadOrbitalInfo_Interface");
//  app_log()<<"Calling interface->getVersion().  value "<<interface<<endl;
...
//  cout<<"ReadOrbitalInfo 57.  Rank = "<< OHMMS::Controller->rank()<<endl;
  ...
  app_log() << buff;
//  cout<<"ReadOrbitalInfo 71.  Rank = "<< OHMMS::Controller->rank()<<endl;
  OHMMS::Controller->barrier();
  snprintf (buff, 1000,
            "  SuperLattice = \n    [ %9.6f %9.6f %9.6f\n"
            "      %9.6f %9.6f %9.6f\n"
  ...
  CheckLattice();
//    cout<<"ReadOrbitalInfo 80.  Rank = "<< OHMMS::Controller->rank()<<endl;
  OHMMS::Controller->barrier();
...
//  cout<<"ReadOrbitalInfo100.  Rank = "<< OHMMS::Controller->rank()<<endl;
  OHMMS::Controller->barrier();
  ...
  //////////////////////////////////
//  app_log()<<"Species ID's"<<endl;
  ParticleSet::ParticleIndex_t species_ids;
  esinterface->getSpeciesIDs(species_ids);
//  app_log()<<"speciesids: "<<species_ids[0]<<" "<<species_ids[1]<<" "<<species_ids[2]<<" "<<species_ids[3]<<" "<<species_ids[4]<<" "<<species_ids[5]<<endl;
...
//  }
//  app_log()<<"Get Atomic Numbers"<<endl;

  esinterface->getAtomicNumbers(atomic_numbers);
//  app_log()<<"QMCPACK SAYS "<< atomic_numbers[0]<<" "<<atomic_numbers[1]<<endl;
...
//  app_log()<<"get Ion Positions"<<endl;
...
void EinsplineSetBuilder::OccupyBands_Interface(int spin, int sortBands, int numOrbs)
{
  ...
  app_log()<<" spin="<<spin<<" sortBands="<<sortBands<<" numOrbs="<<numOrbs<<endl;
  for (int ti=0; ti<DistinctTwists.size(); ti++)
  {
    app_log()<<"Doop\n";
    app_log()<<"DistinctTwist[ti]="<<ti<<" "<<DistinctTwists[ti]<<endl;
...
EinsplineSetBuilder::createSPOSetFromInterface(xmlNodePtr cur)
{
  ...
  int sortBands(1);
  int spinSet = 0;
  cout<<"createSPOSetFromInterface 915.  Rank = "<< OHMMS::Controller->rank()<<endl;
  OHMMS::Controller->barrier();
  app_log()<<"barrier\n";
  
  string ifacename("interface"); //Only one supported interface name at a time.  Hard coded for now.  
  string sourceName;
...
// app_log()<<"put called\n";
 ...
 // ESHDF5Interface* interface = new ESHDF5Interface(myComm);
  //ESPWSCFInterface* interface = new ESPWSCFInterface(myComm);
 // esinterface->put(XMLRoot);
 // esinterface->initialize();
  //ESInterfaceBase* esinterface=static_cast<ESInterfaceBase*>(BasisSetBuilder::interface);
  app_log()<<"esinterface="<<esinterface<<" BasisSetBuilder::interface="<<BasisSetBuilder::interface<<endl;
  cout<<"createSPOSetFromInterface 961.  Rank = "<< OHMMS::Controller->rank()<<endl;
  OHMMS::Controller->barrier();
...
#endif
  //Not really an "H5OrbSet", but using the interface name instead of h5file name to distinguish
  //between different electron spin/orbital sets.  should be fine.   
  H5OrbSet aset(ifacename, spinSet, numOrbs, sortBands);
  ...
  char buff[1000];
  cout<<"createSPOSetFromInterface 1057.  Rank = "<< OHMMS::Controller->rank()<<endl;
  OHMMS::Controller->barrier();
  if (myComm->rank() == 0)
  {
  ...
  if (numOrbs == 0)
  {
    ...
    app_log() << "  Reading " << numOrbs << " orbitals from PW interface.\n";
  mytimer.restart();
  cout<<"createSPOSetFromInterface 1077.  Rank = "<< OHMMS::Controller->rank()<<endl;
  OHMMS::Controller->barrier();
  app_log()<<"mytimer.restart()"<<endl;
  app_log()<<" before readorbital info.  interface address = "<<esinterface<<endl;
  /////////////////////////////////////////////////////////////////
...
//  if (myComm->rank() == 0)
  //  if(!ReadOrbitalInfo())
    ...
    {
      app_error() << "Error reading orbital info from PW Interface.  Aborting.\n";
      APP_ABORT("EinsplineSetBuilder::createSPOSet");
    }
...
//          else
            MixedSplineReader= new SplineAdoptorReaderInterface<SplineR2RAdoptor<float,double,3> >(this,esinterface);
        }
        else
        {
//          if(TargetPtcl.Lattice.SuperCellEnum == SUPERCELL_OPEN)
...
//          else
            MixedSplineReader= new SplineAdoptorReaderInterface<SplineR2RAdoptor<double,double,3> >(this,esinterface);
        }
      }
      else
      {
  ...
  else
  {
    if(MixedSplineReader==0)
    {
      ...
      if(use_single)
      {
        ...
        MixedSplineReader= new SplineAdoptorReaderInterface<SplineC2CPackedAdoptor<float,double,3> >(this,esinterface);
#else
        MixedSplineReader= new SplineAdoptorReaderInterface<SplineC2RPackedAdoptor<float,double,3> >(this,esinterface);
#endif
      }
      else
      {
#if defined(QMC_COMPLEX)
        MixedSplineReader= new SplineAdoptorReaderInterface<SplineC2CPackedAdoptor<double,double,3> >(this,esinterface);
#else
        MixedSplineReader= new SplineAdoptorReaderInterface<SplineC2RPackedAdoptor<double,double,3> >(this,esinterface);
#endif
      }
    }
    ...
    HasCoreOrbs=bcastSortBands(spinSet,NumDistinctOrbitals,myComm->rank()==0);
    cout<<"EinsplineSetBuilder_Interface::createSPOSet spline.  spinset="<<spinSet<<" myrank="<<myComm->rank()<<endl;
    SPOSetBase* bspline_zd=MixedSplineReader->create_spline_set(spinSet,spo_cur);
    if(bspline_zd)
      SPOSetMap[aset] = bspline_zd;
    else
...
bool EinsplineSetBuilder::ReadGvectors_Interface()
{
  ...
  hasPsig = esinterface->getHasPsiG();
  esinterface->getMeshSize(MeshSize);
  if(hasPsig)
  {
    int nallowed=257;
    int allowed[] =
...
//        h_Gvecs.read (H5FileID, Gpath.str().c_str()); //Ray:  HWI
        esinterface->getReducedGVecs(Gvecs);
        numg=Gvecs[0].size(); //Ray:  Replace with interface. 
        cout<<"root:  numg = "<<numg<<endl;
        cout<<"maxindex "<<maxIndex[0]<<" "<<maxIndex[1]<<" "<<maxIndex[2]<<endl;
 //     }
 //     myComm->bcast(numg);
 //     if(!root)
 //       Gvecs[0].resize(numg);
 ...
 //       cout<<"Gvecs["<<ig<<"] ("<<OHMMS::Controller->rank()<<") = "<<Gvecs[0][ig][0]<<" "<<Gvecs[0][ig][1]<<" "<<Gvecs[0][ig][2]<<endl;
  //      cout<<"maxindex["<<ig<<"] ("<<OHMMS::Controller->rank()<<") = "<<maxIndex[0]<<" "<<maxIndex[1]<<" "<<maxIndex[2]<<endl;
      }
...
//          h_Gvecs.read (H5FileID, Gpath.str().c_str()); //Ray:  HWI
          esinterface->getReducedGVecs(Gvecs,ik);

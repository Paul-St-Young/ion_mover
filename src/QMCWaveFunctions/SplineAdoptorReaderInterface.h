...
struct SplineAdoptorReaderInterface: public BsplineReaderBase
{
  ...
  //Holding temporary data for ion updates.
  Vector<complex<double> > cG;
//  vector<BandInfo>& cur_bands; 
  Array<DataType,3> data_r, data_i;
  
  SplineAdoptorReaderInterface(EinsplineSetBuilder* e, ESInterfaceBase* intfc)
    : BsplineReaderBase(e), bspline(0),FFTplan(NULL), esinterface(intfc), cG(0)
  {}
  ...
  SPOSetBase* create_spline_set(int spin, const BandInfoGroup& bandgroup)
  {
    ...
    if(root)
    {
      ...
      if(foundspline)
      {
        ...
        app_log()<<"foundspline1 = "<<foundspline<<" myrank="<<myComm->rank()<<endl;
      }
      if(foundspline)
      {
        int sizeD=0;
        ...
        app_log()<<"foundspline2 = "<<foundspline<<" myrank="<<myComm->rank()<<endl;
      }
      if(foundspline)
    ...
    else
    {
      ...
      //  fftw_destroy_plan(FFTplan);  //DON'T DESTROY THE PLAN.  WE WILL REUSE.  
     //   FFTplan=NULL;
      }
      else//why, don't know
...
//	  stringstream ss;
//	  ss<<"====Band = "<<iorb<<" ====\n";
//	  for (int igk=0; igk<ngvec; igk++)
//  	  {
//	    ss<<igk<<" "<<mybuilder->Gvecs[0][igk][0]<<" "<<mybuilder->Gvecs[0][igk][1]<<" "<<mybuilder->Gvecs[0][igk][2]<<" "<<abs(cG[igk])<<" "<<arg(cG[igk])<<endl;
//	  }
	  
//          if (myComm->rank()==0) app_log()<<ss.str()<<endl;
	  unpack4fftw(cG,mybuilder->Gvecs[0],MeshSize,FFTbox);
	  fftw_execute (FFTplan);
  ...
  void read_spline_set( SPOSetBase* spo, int spin, const BandInfoGroup& bandgroup)
  {
    BsplineSet<adoptor_type>* myspline = dynamic_cast<BsplineSet<adoptor_type>* >(spo);
    int N = bandgroup.getNumDistinctOrbitals(); 
    //assuming that the FFT grid hasn't changed size.  
    for(int iorb=0; iorb<N; ++iorb)
      {
	  int ti=cur_bands[iorb].TwistIndex;
	  ...
	  myspline->set_spline(data_r.data(),data_i.data(),ti,iorb,0);
	}
  }

  void initialize_spline_pio(int spin, const BandInfoGroup& bandgroup)
  {
    app_log()<<"SplineAdoptorReaderP::initialize_spline_pio\n";
    int N=bandgroup.getNumDistinctOrbitals();
    bool root=(myComm->rank()==0);
    bool foundit=true;
    int np=OrbGroups.size()-1;
    app_log()<<"!!!!Inside initialize_spline_pio in SARInterface "<<myComm->rank()<<endl;
    if(myComm->rank()<np)
    {
...
//        if (myComm->rank()==0) app_log()<<ss.str()<<endl;
  ...
  {
    app_log()<<"SplineAdoptorReaderP::initialize_spline_pio_bcast\n";
    bool root=(myComm->rank()==0);
    int np=OrbGroups.size()-1;
    bool foundit=true;
    const vector<BandInfo>& cur_bands=bandgroup.myBands;
  ...
  {
    app_log()<<"SplineAdoptorReaderP::initialize_spline_psi_r\n";
  ...
  inline bool set_grid_interface(const TinyVector<int,3>& halfg, GT* xyz_grid, BCT* xyz_bc)
  {
    ...
    bool havePsig=mybuilder->ReadGvectors_Interface();

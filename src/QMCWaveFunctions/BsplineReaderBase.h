...
struct BsplineReaderBase
{
  ...
  ///Map of needed orbitals (Nup and Ndown for single slater), indexed by "spin".
  //Map is chosen because it simplifies creation and ordering of these parameters.
  std::map<int,int> NumOrbs;
  ...
  /** read the orbitals into an SPOSetBase.  This assumes meshes and the like have already been initialized.
   */
  void read_spline_set(SPOSetBase* spo, int spin); 
  void read_spline_set(SPOSetBase* spo, int spin, const BandInfoGroup& bandgroup){APP_ABORT("read_spline_set not specialized");}; 

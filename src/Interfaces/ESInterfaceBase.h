...
//    void setSimulationCell(SimulationCell);
    ...
    virtual void getSpeciesData(Vector<int>& am, Vector<int>& charges, Vector<double>& masses, Vector<string>& names)=0;
    ...
    ///Electron Info
    virtual void getNumElectrons(vector<int>& num_spins)=0;
    virtual void setNumElectrons(const vector<int>& num_spins){};
    ...
    virtual void getOrbEigenvals(const int spin, const int kid, vector<double> & eigenvals)=0;
    virtual void setOrbEigenvals(const int spin, const int kid, const vector<double> & eigenvals){};
    ///ORBITAL DATA
    virtual int getHasPsiG()=0;
    
    virtual void getMeshSize(TinyVector<int,3> & mesh)=0;
     
    virtual void getReducedGVecs(vector<vector<TinyVector<int,3> > > & gvecs, int index=0)=0;
    
         
    virtual bool getPsi_kspace(Vector<complex<double> > & Cg, int spin, int orbid, int twistid)=0;
//    virtual bool getPsi_rspace(data, int orbid, int twistid){return true;};
 ...
 //   void setDensityReducedGvecs(const int){};
    ...
    //ORBITAL DATA
    virtual void getAtomicOrbitals(std::vector<AtomicOrbital<complex<double> > > & AtomicOrbitals)=0;
    virtual void setAtomicOrbitals(const std::vector<AtomicOrbital<complex<double> > > & AtomicOrbitals){};

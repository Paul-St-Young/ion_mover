...
*/
  ...
  {
  public:
    ESHDF5Interface(Communicate* mycomm):ESInterfaceBase(mycomm), myComm(mycomm), H5FileName(""),H5FileID(0),box_changed(false), pos_changed(false)        {ifacename = "ESHDF5";};
    ~ESHDF5Interface(){};
...
//    void setSimulationCell(SimulationCell){};
    ...
    void getNumElectrons(vector<int>& num_spins);
    ...
    void getAtomicOrbitals(vector<AtomicOrbital<complex<double> > > & AtomicOrbitals);
    void setAtomicOrbitals(const vector<AtomicOrbital<complex<double> > > & AtomicOrbitals){};

...
void ESHDF5Interface::getNumElectrons(vector<int>& num_spins)
{
  HDFAttribIO<vector<int> > h_numspins(num_spins);
...
void ESHDF5Interface::getSpeciesData(Vector<int>& am, Vector<int>& charge, Vector<double>& mass, Vector<string>& names)
{
  ...
  {
    ostringstream anumname;
    ostringstream chargename;
    ostringstream massname;
    ...
    HDFAttribIO<string> h_species_name (names[isp]);
...
void ESHDF5Interface::getAtomicOrbitals(vector<AtomicOrbital<complex<double> > > & AtomicOrbitals)
{
...
void ESHDF5Interface::getOrbEigenvals(const int spin, const int kid, vector<double>& eigenvals)
{
    ostringstream ePath;
    ePath << "/electrons/kpoint_" << kid << "/spin_"
          << spin << "/eigenvalues";
    Vector<double> eigvals;
    HDFAttribIO<vector<double> > h_eigvals(eigenvals);
...
void ESHDF5Interface::getReducedGVecs(vector<vector<TinyVector<int,3> > > & gvecs, int index)
{
    ostringstream Gpath;  //Ray:  HWI
    Gpath    << "/electrons/kpoint_"<<index<<"/gvectors";  //Ray:  HWI 
    HDFAttribIO<vector<TinyVector<int,3> > > h_Gvecs(gvecs[index]); //Ray:  HWI
    h_Gvecs.read (H5FileID, Gpath.str().c_str()); //Ray:  HWI

    return;
}

//bool ESHDF5Interface::getPsi_kspace(Vector<complex<double> > & Cg, int orbid, int twistid)
//{
...
bool ESHDF5Interface::getPsi_kspace(Vector<complex<double> > & cG,int spin, int orbid, int twistid)
{
        string s=psi_g_path(twistid,spin,orbid); //Ray:  HWI (Handle with interface)

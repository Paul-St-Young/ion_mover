...
*/
  ...
  public:
    ESPWSCFInterface(Communicate* mycomm):ESInterfaceBase(mycomm), myComm(mycomm), box_changed(false), pos_changed(false),
					  ...
					  npw(-1), npwx(-1), nelec(-1), nup(-1), ndown(-1) 
					 {mesh[0]=mesh[1]=mesh[2]=-1; ifacename="ESPWSCF";}; 
   ~ESPWSCFInterface(){};
...
//    void setSimulationCell(SimulationCell){};
    ...
    int getNumAtoms();
    void getNumElectrons(vector<int>& numspins);
    ...
    void getSpeciesData(Vector<int>& am, Vector<int>& charges, Vector<double>& masses, Vector<string>& names);
    ...
    void getAtomicOrbitals(std::vector<AtomicOrbital<complex<double> > > & AtomicOrbitals);
    void setAtomicOrbitals(const std::vector<AtomicOrbital<complex<double> > > & AtomicOrbitals){};

    void getOrbEigenvals(const int spin, const int kid, vector<double> & eigenvals);
    void setOrbEigenvals(const int spin, const int kid, const vector<double> & eigenvals){}; 
    
    int getHasPsiG();
    
    void getMeshSize(TinyVector<int,3> & mesh);
     
    void getReducedGVecs(vector<vector<TinyVector<int,3> > > & gvecs, int index=0);
    
    bool getPsi_kspace(Vector<complex<double> > & Cg, int spin, int orbid, int twistid);
  ...
  private:
    ...
    vector<double> species_masses; 
    vector<double> species_charges;
   ...
   */
   ...
   inline Vector<string> convertCharToString(char namelist[][PW_NAME_LEN], int num_names)
   {

      Vector<string> namevec(num_names);
      for(int i=0; i<num_names; i++)
      {
        string tmpstr(namelist[i]);

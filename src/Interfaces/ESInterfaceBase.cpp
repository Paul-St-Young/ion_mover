...
void ESInterfaceBase::getParticleSet(ParticleSet & P)
{
  ...
  app_log()<<"atomicNumbers.size()="<<atomicNumbers.size()<<endl;
  app_log()<<"speciesNames.size()="<<speciesNames.size()<<endl;
  app_log()<<"speciesMass.size()="<<speciesMass.size()<<endl;
  app_log()<<"speciesCharge.size()="<<speciesCharge.size()<<endl;
  app_log()<<"P.groupID.size()="<<P.GroupID.size()<<endl;
  app_log()<<"P.R.size()="<<P.R.size()<<endl;
  app_log()<<"P.ID.size()="<<P.ID.size()<<endl;
  app_log()<<"P.getTotalNum()="<<P.getTotalNum()<<endl;
  P.PrimitiveLattice=P.Lattice;
...
{
  myComm->barrier();
  cout<<"ESInterfaceBase::DEBUGGGG mycomm="<<myComm<<" size="<<myComm->size()<<" rank="<<myComm->rank()<<endl;
  SpeciesSet& tspecies(P.getSpeciesSet());
  if(myComm->size()==1)
    return; 
  int nspecies=tspecies.getTotalNum();
  ...
  myComm->bcast(bsizes);
  myComm->barrier();
  cout<<"ESInterfaceBase:: made it through barrier o= "<<o.str()<<" "<<bsizes[2]<<" "<<myComm<<" size="<<myComm->size()<<" rank="<<myComm->rank()<<endl;
  ...
  cout<<"ESInterfaceBase:: before bcast species_names "<< species_names<<" mycomm="<<myComm<<" size="<<myComm->size()<<" rank="<<myComm->rank()<<endl;
  
  myComm->bcast(species_names,bsizes[2]);
  cout<<"ESInterfaceBase:: after bcast species_names  mycomm="<<myComm<<" size="<<myComm->size()<<" rank="<<myComm->rank()<<endl;
  myComm->barrier();
  if(myComm->rank()==0)
  {
    vector<string> vlist;
 ...
 //   P.create(bsizes[1]);
  ...
  for(int i=0; i<tspecies.numAttributes(); ++i)
    pbuffer.add(tspecies.d_attrib[i]->begin(),tspecies.d_attrib[i]->end());
  pbuffer.add(lat.begin(),lat.end());
  cout<<"ESInterfaceBase:: lat len = "<<lat.end()-lat.begin()<<" mycomm="<<myComm<<" size="<<myComm->size()<<" rank="<<myComm->rank()<<endl;
  pbuffer.add(get_first_address(P.R),get_last_address(P.R));
  cout<<"ESInterfaceBase:: p.r len = "<<get_last_address(P.R)-get_first_address(P.R)<<" mycomm="<<myComm<<" size="<<myComm->size()<<" rank="<<myComm->rank()<<endl;
  pbuffer.add(P.GroupID.begin(),P.GroupID.end());
  cout<<"ESInterfaceBase:: pgroupid len = "<<P.GroupID.end()-P.GroupID.begin()<<" mycomm="<<myComm<<" size="<<myComm->size()<<" rank="<<myComm->rank()<<endl;
  cout<<"ESInterfaceBase::Before bcast pbuffer. pbuffer.size="<<pbuffer.size()<<"  mycomm="<<myComm<<" size="<<myComm->size()<<" rank="<<myComm->rank()<<endl;

...
void ESHDFIonsParser::getParticleSet(ParticleSet & P)
{
  ...
  }
  {
    ...
    HDFAttribIO<ParticleSet::ParticlePos_t> b(P.R);
    b.read(hfile_id,"atoms/positions");

    P.applyBC(P.R); //force them [0,1)

    HDFAttribIO<ParticleSet::ParticleIndex_t> c(P.GroupID);
...
void ESHDFIonsParser::bcastParticleSet(ParticleSet& P)
{
  SpeciesSet& tspecies(P.getSpeciesSet());
  if(myComm->size()==1)
    return; 
  ...
  if(myComm->rank())
  {
    ...
    pbuffer.get(lat.begin(),lat.end());
    pbuffer.get(get_first_address(ref_.R),get_last_address(ref_.R));
    pbuffer.get(ref_.GroupID.begin(),P.GroupID.end());
    P.Lattice.set(lat);
  }
  return ;
